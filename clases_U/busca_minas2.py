...
# Funcion para darle posiciones aleatorias a las bombas, asegurándose de que no se repitan
def posicion_bombas():
    num_bombas = random.randint(1, 3)  # Selecciona aleatoriamente cuántas bombas habrá, entre 1 y 3
    bombas = set()  # Usamos un conjunto para asegurarnos de que las bombas no se repitan
    while len(bombas) < num_bombas:  # Queremos entre 1 y 3 bombas
        fila = random.randint(0, 3)
        columna = random.randint(0, 3)
        bombas.add((fila, columna))  # Agrega una posición única
    return list(bombas)  # Convertimos el conjunto en lista para usar las posiciones
    ...
    for i in range(fila - 1, fila + 2):  # Recorrer las filas adyacentes
        for j in range(columna - 1, columna + 2):  # Recorrer las columnas adyacentes
            if 0 <= i < 4 and 0 <= j < 4:  # Asegurarse de que las coordenadas estén dentro de la matriz
                if matriz[i][j] == "B":
                    cant_bombas += 1
    matriz[fila][columna] = str(cant_bombas)  # Coloca el número de bombas en la casilla seleccionada
    return True
...
def ejecutar():
    ...
    bombas = posicion_bombas()  # Genera entre 1 y 3 posiciones de bombas aleatorias
    for bomba in bombas:  # Coloca las bombas en las posiciones seleccionadas
        bomba_fila, bomba_columna = bomba
        matriz[bomba_fila][bomba_columna] = "B"
    ...
    while turno <= 16:
        ...
        # Si el jugador selecciona una bomba, el juego termina
        if matriz[fila][columna] == "B":
            print(f"¡BOOM! Jugador {turno} ha perdido.")
            break
